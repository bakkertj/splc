...
Entry layout emitted:  { "word", flags, polarity, "stress1|stress2" }
...
def load_cmu():
    ...
    with open(os.path.join(DATA, "cmudict.dict"), encoding="latin-1") as f:
        for line in f:
            ...
            s = "".join(ch for ph in parts[1:] for ch in ph if ch.isdigit()).replace("2", "1")
...
def load_names():
    ...
    return {l.strip().lower() for l in open(p) if l.strip()} if os.path.exists(p) else set()
...
def main():
    ...
    words = set(stress) | nouns | adjs | set(pol) | names | set(ov) | FUNCTION_WORDS
    entries = []
    ...
    with open(OUT, "w") as out:
        ...
        for w, f, p, s in entries:
            out.write('{"%s",%d,%d,"%s"},\n' % (w.replace('\\', '\\\\').replace('"', '\\"'), f, p, s))
