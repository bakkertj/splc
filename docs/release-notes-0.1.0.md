...
`Hi!` and `Ho!` and passes `-fsonnet -fpentameter=error`. `examples/balcony.spl` prints Juliet's
balcony soliloquy in prose, and `examples/balcony_verse.spl`, generated by
`tools/text_to_spl.py --verse`, prints the same text in rhymed iambic pentameter and passes
`-fpentameter=error -frhyme-scheme=AABB`.
...
* `tools/text_to_spl.py` turns a text file into a play that prints it, in prose or, with
  `--verse`, in rhymed pentameter.
...
LLVM 18.1.3 and 20.1.2 and on macOS (Apple silicon) with Homebrew LLVM 20.1.8 and 23.1.0;
`build_mac.sh` does the Homebrew configuration for you. CI runs the seventeen tests and a
calibration check on Ubuntu (apt LLVM 18) and macOS (Homebrew LLVM, currently 23).

The LLVM 21 and later API changes (Triple-typed target lookup) and an LLVM 23 behaviour
change in `BasicBlock::getTerminator()` on empty blocks are handled in this release.
